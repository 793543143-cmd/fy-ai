...
function resolveAppPath(...segments) {
  ...
  if (path.isAbsolute(first)) return path.join(first, ...rest);
  return path.join(__dirname, first, ...rest);
}
...
const ALLOW_RUNTIME_KEY_WRITE = envFlag(process.env.FYAI_ALLOW_RUNTIME_KEY_WRITE, APP_ENV !== "production");
const SAVE_OUTPUTS = envFlag(process.env.FYAI_SAVE_OUTPUTS, APP_ENV !== "production");
const REQUIRE_EXPLICIT_ADMIN_PASSWORD = envFlag(process.env.FYAI_REQUIRE_ADMIN_PASSWORD, APP_ENV === "production");
const COOKIE_SECURE = envFlag(process.env.FYAI_COOKIE_SECURE, APP_ENV === "production");
...
async function writeLocalEnvValue(name, value) {
  if (!ALLOW_RUNTIME_KEY_WRITE) {
    const error = new Error("当前部署已关闭网页写入密钥，请到部署平台环境变量里修改。");
    error.status = 403;
    throw error;
  }
...
function ensureAccessStore(store) {
  ...
  if (!store.users.length) {
    const initialAdminPassword = process.env.FYAI_ADMIN_PASSWORD || (!REQUIRE_EXPLICIT_ADMIN_PASSWORD ? "FYAI2026!" : "");
    if (!initialAdminPassword) {
      const error = new Error("生产部署前请设置 FYAI_ADMIN_PASSWORD，避免公网实例使用默认管理员密码。");
      error.status = 500;
      throw error;
    }
    // 新增登录授权：首次启动自动生成管理员，保证部署后可以立即进入后台初始化。
    store.users.push({
      id: id("user"),
      username: "admin",
      name: "管理员",
...
function isLocalRequest(req) {
  if (!ALLOW_LOCAL_ADMIN_BYPASS) return false;
...
function sessionCookie(token, maxAge = 14 * 24 * 60 * 60) {
  return `fyai_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${maxAge}${COOKIE_SECURE ? "; Secure" : ""}`;
}

function recordDownload(job, meta = {}) {
  job.downloads = Array.isArray(job.downloads) ? job.downloads : [];
  job.downloads.push({
    downloadedAt: now(),
    filename: meta.filename || safeDownloadName(job, meta.extension || ".mp4"),
...
function safeDownloadName(job, extension = ".mp4") {
  const owner = sanitizeString(job.owner) || "未填写";
  const category = sanitizeString(job.category) || "未分类";
  const batchName = sanitizeString(job.batchName) || sanitizeString(job.scriptTitle) || job.id;
  const variantIndex = Number(job.variantIndex);
  const sequenceLabel = Number.isFinite(variantIndex) && variantIndex > 0
    ? `第${variantIndex}条`
...
async function handleApi(req, res, url) {
  ...
  if (method === "GET" && url.pathname === "/api/state") {
    return jsonResponse(res, 200, {
      ...
      deployment: {
        appEnv: APP_ENV,
        allowLocalAdminBypass: ALLOW_LOCAL_ADMIN_BYPASS,
        allowRuntimeKeyWrite: ALLOW_RUNTIME_KEY_WRITE,
        saveOutputs: SAVE_OUTPUTS
      },
  ...
  if (method === "POST" && url.pathname === "/api/settings/poyo-key") {
    if (currentUser.role !== "admin") return jsonResponse(res, 403, { error: "只有管理员可以修改密钥" });
    const body = await readJson(req);
  ...
  if (method === "POST" && url.pathname.match(/^\/api\/jobs\/[^/]+\/download$/)) {
    ...
    if (!SAVE_OUTPUTS) {
      return jsonResponse(res, 409, { error: "当前部署没有开启服务器本地保存，请直接点击保存下载到你的电脑。", job });
    }
    ...
    const friendlyName = safeDownloadName(job, extension);
    ...
    recordDownload(job, { filename: friendlyName, extension, mode: "server-save" });
    await writeStore(store);
    return jsonResponse(res, 200, { job, summary: summarize(store) });
  }
  ...
  if (method === "GET" && url.pathname.match(/^\/api\/jobs\/[^/]+\/download-file$/)) {
    ...
    if (localFile?.path) {
      const localPath = path.join(OUTPUT_DIR, path.basename(localFile.path));
      if (!localPath.startsWith(OUTPUT_DIR)) return textResponse(res, 403, "Forbidden");
      const data = await fs.readFile(localPath);
      const ext = path.extname(localPath) || ".mp4";
      const downloadName = localFile.name || safeDownloadName(job, ext);
      recordDownload(job, { filename: downloadName, extension: ext, mode: "cached-file" });
      await writeStore(store);
      res.writeHead(200, {
        "Content-Type": localFile.contentType || MIME_TYPES[ext] || "application/octet-stream",
        "Content-Length": data.byteLength,
        "Content-Disposition": downloadDisposition(downloadName)
      });
      return res.end(data);
    }
    ...
    const downloadName = safeDownloadName(job, ext);
    recordDownload(job, { filename: downloadName, extension: ext, mode: "stream" });
    await writeStore(store);
    res.writeHead(200, {
...
async function serveStatic(res, url) {
  ...
  const filePath = safePath.startsWith("/outputs/")
    ? path.join(OUTPUT_DIR, path.basename(safePath))
    : path.join(__dirname, safePath);
  const allowedRoot = safePath.startsWith("/outputs/") ? OUTPUT_DIR : __dirname;
  if (!filePath.startsWith(allowedRoot)) return textResponse(res, 403, "Forbidden");
  try {
