...
let uploadedImages = [];
let mentionStart = -1;
let justSubmittedJobIds = new Set();
...
function safeDownloadFilename(job) {
  const extension = job.files?.find((item) => item.file_type === "video")?.format
    ...
    : ".mp4";
  const batchLabel = String(job.batchName || job.scriptTitle || job.id || "").trim();
  const sequenceLabel = Number.isFinite(Number(job.variantIndex)) && Number(job.variantIndex) > 0
    ? `第${Number(job.variantIndex)}条`
    : `任务${String(job.id || "").slice(-4) || "1"}`;
  const name = [
...
function hasDownloadRecord(job) {
  return (Array.isArray(job.downloads) && job.downloads.length > 0)
    || (Array.isArray(job.localFiles) && job.localFiles.some((item) => item?.path));
}
...
function render() {
  ...
  const canWriteKeys = Boolean(state.deployment?.allowRuntimeKeyWrite);
  const canManageKeys = canWriteKeys && (currentUser?.role === "admin" || (state.deployment?.allowLocalAdminBypass && isLocalAdminPage));
  ...
  ["#keyForm", "#arkKeyForm", "#volcAkSkForm"].forEach((selector) => {
    const form = $(selector);
    if (!form) return;
    $$(`${selector} input, ${selector} button`).forEach((field) => {
      field.disabled = !canWriteKeys;
    });
  });
  const keyMeta = $("#keyModalMeta");
  if (keyMeta) {
    keyMeta.textContent = canWriteKeys
      ? "管理员专用。保存后会立即更新当前接口，不需要重启服务。"
      : "当前部署已关闭网页改密钥，请到部署平台环境变量里修改。";
  }
  updateSubmitState();
...
function renderMetrics() {
  ...
  const draftUsage = state.jobs
    .filter((job) => job.status === "draft")
    .reduce((map, job) => addUsage(map, job.model, job.estimatedCredits), emptyUsageMap());
  const cards = [
    ["提示词", state.summary?.totalScripts || 0],
    ["批次", state.summary?.totalBatches || 0],
    ["任务总数", state.summary?.totalJobs || 0],
    ["PoYo余额", `${formatCredits(state.accountBalance?.credits ?? "未同步")} 积分`],
    ["草稿预计用量", formatUsageMap(draftUsage)],
...
function usageUnitForModel(model) {
  return state.pricingRules?.[model]?.unit || (modelProvider(model) === "ark" ? "tokens" : "积分");
}

function usageBucketForModel(model) {
  return modelProvider(model) === "ark" ? "ark" : "poyo";
}
...
function formatUsageValue(value, model) {
  const formatted = formatCredits(value);
  return formatted ? `${formatted} ${usageUnitForModel(model)}` : "";
}
...
function addUsage(map, model, value) {
  const amount = Number(value);
  if (!Number.isFinite(amount) || amount <= 0) return map;
  map[usageBucketForModel(model)] = (map[usageBucketForModel(model)] || 0) + amount;
  return map;
}

function mergeUsageMaps(rows, key) {
  return rows.reduce((total, row) => {
    total.poyo += row[key]?.poyo || 0;
    total.ark += row[key]?.ark || 0;
    return total;
  }, emptyUsageMap());
}

function usageMapValue(map) {
  return (map?.poyo || 0) + (map?.ark || 0);
}

function formatUsageMap(map) {
  const parts = [];
  if (map?.poyo) parts.push(`${formatCredits(map.poyo)} 积分`);
  if (map?.ark) parts.push(`${formatCredits(map.ark)} tokens`);
...
function renderCostPreview() {
  ...
  const unitLabel = usageUnitForModel(model);
  const isPoyo = modelProvider(model) !== "ark";
  const balance = state.accountBalance?.credits;
  const balanceText = isPoyo && Number.isFinite(Number(balance)) ? `${formatCredits(balance)} 积分` : (isPoyo ? "未同步" : "火山后台结算");
  const enough = !isPoyo || totalCredits === null || !Number.isFinite(Number(balance)) || Number(balance) >= totalCredits;
  $("#costPreview").innerHTML = `
    <span class="${enough ? "" : "warn"}">预计用量 <strong>${totalCredits === null ? "待确认" : `${formatCredits(totalCredits)} ${unitLabel}`}</strong></span>
    <span>本批 ${totalJobs} 条</span>
    <span>单条 ${unitCredits === null ? "待确认" : `${formatCredits(unitCredits)} ${unitLabel}`}</span>
    <span>${isPoyo ? "余额" : "结算"} ${balanceText}</span>
  `;
}
...
function renderPricingList() {
  const rows = Object.entries(state.pricingRules || {}).map(([model, rule]) => {
    let price = "";
    const unitLabel = rule.unit || (rule.provider === "ark" ? "tokens" : "积分");
    if (rule.billing === "duration_tier") {
      price = Object.entries(rule.durations || {}).map(([seconds, credits]) => `${seconds}s=${credits}${unitLabel}`).join(" · ");
    } else if (rule.billing === "resolution_per_second") {
      price = Object.entries(rule.rates || {}).map(([resolution, credits]) => `${resolution}=${credits}${unitLabel}/秒`).join(" · ");
    } else if (rule.billing === "flat") {
      price = `${rule.credits}${unitLabel}/条`;
    }
...
function getKnownOwners() {
  // 体验优化：使用人来源统一，已批准的新账号会自动进入生成页、流水筛选和用量统计。
...
function renderJobs() {
  ...
  body.innerHTML = jobs
    .map((job) => {
      ...
      const downloaded = hasDownloadRecord(job);
      ...
      const canSubmit = (job.provider || state.pricingRules?.[job.model]?.provider || "poyo") === "ark"
        ? state.hasArkKey && ["draft", "failed", "submit_failed"].includes(job.status)
        : state.hasPoyoKey && ["draft", "failed", "submit_failed"].includes(job.status);
      const submitLabel = ["failed", "submit_failed"].includes(job.status) ? "重试" : "提交";
      return `
        <tr class="${justSubmittedJobIds.has(job.id) ? "job-highlight" : ""}">
          <td>
          ...
          <td>
            ...
            <div class="job-sub"><span class="download-state ${downloaded ? "downloaded" : "pending"}">${downloaded ? "已下载" : "未下载"}</span></div>
          </td>
          <td>
            <div>${escapeHtml(formatUsageValue(job.actualCredits ?? job.estimatedCredits ?? "", job.model))}</div>
            <div class="job-sub">${job.actualCredits ? "实际用量" : "预计用量"}</div>
          </td>
          <td>
            <div class="row-actions">
              <button class="tiny ${submitLabel === "重试" ? "retry" : ""}" data-submit="${job.id}" ${canSubmit ? "" : "disabled"} type="button">${submitLabel}</button>
...
function exportFilteredJobsCsv() {
  ...
  const headers = ["使用人", "批次", "提示词", "分类", "模型", "状态", "秒数", "尺寸", "分辨率", "任务ID", "预计用量", "实际用量", "单位", "错误"];
...
// 新增用量统计：直接复用现有任务数据，PoYo 显示积分、火山显示 tokens，避免两个平台口径混算。
...
function getUsageRows() {
  ...
  for (const owner of getKnownOwners()) {
    rows.set(owner, { owner, dayCount: 0, dayCredits: 0, totalCredits: 0, dayUsage: emptyUsageMap(), totalUsage: emptyUsageMap() });
  }
  ...
  for (const job of state.jobs.filter(isUsageJob)) {
    ...
    const row = rows.get(owner) || { owner, dayCount: 0, dayCredits: 0, totalCredits: 0, dayUsage: emptyUsageMap(), totalUsage: emptyUsageMap() };
    const credits = jobUsageCredits(job);
    row.totalCredits += credits;
    addUsage(row.totalUsage, job.model, credits);
    if (isDateInUsageRange(jobUsageDate(job), range, date)) {
      row.dayCount += 1;
      row.dayCredits += credits;
      addUsage(row.dayUsage, job.model, credits);
    }
    rows.set(owner, row);
  }
  ...
  return [...rows.values()]
    ...
    .sort((a, b) => usageMapValue(b.dayUsage) - usageMapValue(a.dayUsage) || b.dayCount - a.dayCount || a.owner.localeCompare(b.owner, "zh-CN"));
}
...
function getUsageBreakdown(groupBy) {
  ...
  for (const job of state.jobs.filter(isUsageJob)) {
    ...
    const row = rows.get(key) || { name: key, count: 0, credits: 0, usage: emptyUsageMap() };
    row.count += 1;
    const usage = jobUsageCredits(job);
    row.credits += usage;
    addUsage(row.usage, job.model, usage);
    rows.set(key, row);
  }
  return [...rows.values()].sort((a, b) => usageMapValue(b.usage) - usageMapValue(a.usage) || b.count - a.count || a.name.localeCompare(b.name, "zh-CN"));
}

function renderBreakdownCard(title, rows) {
  const maxCredits = Math.max(1, ...rows.map((row) => usageMapValue(row.usage)));
  return `
      ...
      <div class="breakdown-list">
        ${rows.length
          ? rows.slice(0, 8).map((row) => {
            const width = Math.max(4, Math.round((usageMapValue(row.usage) / maxCredits) * 100));
            return `
              <div class="breakdown-row">
                <div>
                  <strong>${escapeHtml(row.name)}</strong>
                  <span>${row.count} 条 · ${escapeHtml(formatUsageMap(row.usage))}</span>
                </div>
...
function renderUsageStats() {
  ...
  const dayUsage = mergeUsageMaps(rows, "dayUsage");
  const totalUsage = mergeUsageMaps(rows, "totalUsage");
  const ownerFilter = $("#usageOwnerFilter")?.value || "";
  ...
  $("#usageSummary").innerHTML = [
    ...
    ["当天生成", `${dayCount} 条`],
    ["当天用量", formatUsageMap(dayUsage)],
    ["累计用量", formatUsageMap(totalUsage)],
    ["成功率", successRate]
  ]
    .map(([label, value]) => `<div><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`)
    .join("");

  const maxCredits = Math.max(1, ...rows.map((row) => usageMapValue(row.dayUsage)));
  $("#usageChart").innerHTML = rows.length
    ? rows
      .map((row, index) => {
        const usageValue = usageMapValue(row.dayUsage);
        const height = usageValue > 0 ? Math.max(6, Math.round((usageValue / maxCredits) * 100)) : 0;
        return `
          <div class="usage-bar-item ${usageValue > 0 ? "" : "zero"}">
            <div class="usage-bar-track">
              <span style="height:${height}%; background:${index % 2 ? "#17b7a6" : "#684cff"}"></span>
            </div>
            <strong>${escapeHtml(row.owner)}</strong>
            <em>${escapeHtml(formatUsageMap(row.dayUsage))}</em>
          </div>
        `;
      })
      .join("")
    ...
    ? rows
      .map((row) => `
        <tr>
          ...
          <td>${row.dayCount}</td>
          <td>${escapeHtml(formatUsageMap(row.dayUsage))}</td>
          <td>${escapeHtml(formatUsageMap(row.totalUsage))}</td>
        </tr>
      `)
      .join("")
...
function exportUsageStatsCsv() {
  ...
  const headers = ["统计范围", "用户名", "生成条数", "消耗用量", "总用量"];
  const escapeCsv = (value) => `"${String(value ?? "").replaceAll('"', '""')}"`;
  const lines = [
    headers.map(escapeCsv).join(","),
    ...getUsageRows().map((row) => [
      ...
      row.owner,
      row.dayCount,
      formatUsageMap(row.dayUsage),
      formatUsageMap(row.totalUsage)
    ].map(escapeCsv).join(","))
  ];
  const blob = new Blob([`\ufeff${lines.join("\n")}`], { type: "text/csv;charset=utf-8" });
  const url = URL.createObjectURL(blob);
...
function openJobDetail(jobId) {
  ...
  const rows = [
    ...
    ["下载状态", hasDownloadRecord(job) ? "已下载" : "未下载"],
    ["预计用量", formatUsageValue(job.estimatedCredits ?? "", job.model)],
    ["实际用量", formatUsageValue(job.actualCredits ?? "", job.model)],
    ["视频链接", source.src || "暂无"],
...
async function submitBatchPayload(payload) {
  ...
  try {
    ...
    if (payload.submitNow) {
      justSubmittedJobIds = new Set((result.jobs || []).map((job) => job.id));
      selectedJobIds.clear();
      justSubmittedJobIds.forEach((id) => selectedJobIds.add(id));
      if (location.hash !== "#jobs") location.hash = "#jobs";
      updateViewFromHash();
      toast(`任务已进入队列：${result.jobs.length} 条，已跳到任务流水`);
      setTimeout(() => {
        justSubmittedJobIds.clear();
        renderJobs();
      }, 20000);
    } else {
      toast(`草稿已保存：${result.jobs.length} 条`);
    }
    await loadState();
  } catch (error) {
    toast(error.message);
  ...
  if (payload.submitNow) {
    ...
    const ok = await confirmAction({
      ...
      rows: [
        ...
        ["预计用量", unitCredits === null ? "待确认" : `${formatCredits(unitCredits * totalJobs)} ${usageUnitForModel(payload.model)}`]
      ]
    });
    if (!ok) return;
  }
...
$("#syncBalanceBtn").addEventListener("click", async () => {
  try {
    const result = await api("/api/account/balance", { method: "POST" });
    state.accountBalance = result.accountBalance;
    toast(`PoYo 余额已同步：${formatCredits(result.accountBalance?.credits)} 积分`);
  } catch (error) {
    toast(error.message);
  } finally {
    await loadState();
